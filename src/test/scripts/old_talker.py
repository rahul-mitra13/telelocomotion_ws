...
def callback(data):
	...
	if(tr > 0):
		...
		qy = (m12 + m21) / s
		qz = 0.25 * s
	br.sendTransform((data.point.x, data.point.y, data.point.z),(qw,qx,qy,qz),rospy.Time.now(),"Help2","b")
	
def NO():
	rospy.init_node('old_talker', anonymous=True)
	print("running")
	rospy.Subscriber("/haptic_info2", Data, callback)
	print("bye")
